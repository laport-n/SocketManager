...
export class SessionManager {
  ...
  private async createNewSession(
    ...
    const createSessionEventId = await (
      await this.eventController.saveOne('create new session', 'SESSION_CREATE')
    )._id;
    const sessionId = await (
      await this.sessionController.saveOne(createSessionEventId)
    )._id;
    ...
    if (sessionStringified && JSONUtils.isParsable(sessionStringified)) {
      ...
      if (session) {
        ...
        if (sessionDocument && !sessionDocument.endedAt) {
          ...
          const sessionId = await (
            await this.sessionController.saveOne(eventId)
          )._id;
          socket.data.sessionId = sessionId;
        }
